...
const FILTERS = ['All', 'Processing', 'Needs Information', 'Confirmed'] as const;
...
export default function FieldReports() {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
      <div>
        <h1 className="text-[18px] text-fg">My reports</h1>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={c.label} className="border border-hair bg-raised p-2.5">
            <div className={`font-mono text-[20px] leading-none ${c.tone}`}>{c.value}</div>
      ...
      <div className="flex gap-1.5 overflow-x-auto">
        {FILTERS.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`shrink-0 px-2.5 py-1 border font-mono text-[10px] uppercase tracking-wider ${
              filter === f
      ...
      {error ? (
        ...
        <div className="border border-hair bg-raised py-10 px-4 text-center flex flex-col items-center gap-2">
          <div className="text-[13px] text-fg">No reports yet</div>
          <div className="text-[11px] text-muted">
            Your submitted updates will appear here.
          </div>
          <button
            onClick={() => navigate('/field')}
            className="mt-2 bg-accent text-accent-fg font-mono text-[10px] uppercase tracking-wider px-4 py-2.5"
          >
            Create first report
          </button>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {shown.map((r) => (
            <button
              ...
              className="border border-hair bg-raised p-3 text-left flex flex-col gap-1.5"
            >
              ...
              <span className="text-[12px] text-fg leading-snug">{r.raw_text}</span>
      ...
      {open && (
        <section className="border border-hair bg-surface">
          <div className="px-3 py-2 border-b border-hair flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] text-fg">{open.reference}</span>
          </div>
              ...
              {!open.clarification_response && (
                <div className="px-3 pb-3">
                  <button
                    ...
                    className="w-full bg-accent text-accent-fg font-mono text-[10px] uppercase tracking-wider py-2.5"
                  >
                    Respond now
                  </button>
                </div>
...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="px-3 py-2.5 border-b border-hair flex flex-col gap-1">
      ...
      <span className="text-[12px] text-fg leading-snug">{children}</span>
    </div>
  );
}
