...
export function ConversationStage({
  ...
  const { lang, t, tDynamic, tSug, tVal } = useTranslation();
    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-hair pb-3">
        <span className="text-label font-medium uppercase tracking-[0.05em] text-muted">
          {renderWithSrOnly(t('data_entry_session', 'Data Entry Session'), 'Data Entry Session')}
        </span>
        <Button variant="ghost" size="xs" onClick={onClose} aria-label="Close session">
          {renderWithSrOnly(t('close', 'Close'), 'Close')}
          <X size={14} />
        </Button>
      </div>
      ...
      {stage === 'conversation' && collected.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {collected.map((c) => {
            const chipKey = `chip_${c.label.toLowerCase()}`;
            const labelText = t(chipKey, c.label);
            const valText = tVal(c.value);
            return (
              <span
                key={c.label}
                className="rounded-full bg-selected text-accent px-3 py-1 text-label font-medium flex items-center gap-1"
              >
                <span className="uppercase tracking-[0.05em] opacity-80">{labelText}</span>
                <span>{valText}</span>
                {lang !== 'en-IN' && <span className="sr-only">{c.label} {c.value}</span>}
              </span>
            );
          })}
        </div>
      ...
      {stage === 'conversation' && lastSaid && (
        <div className="flex flex-col gap-1">
          ...
          <p className="text-lead leading-6 text-fg">{tVal(lastSaid)}</p>
        </div>
      )}
      ...
      {stage === 'conversation' && currentQuestion && !thinking && (
        <div className="border border-hair bg-raised rounded-lg px-5 py-5 flex flex-col gap-4">
          <span className="flex items-center gap-2 text-label font-semibold uppercase tracking-[0.05em] text-accent">
            <Cpu size={14} />
            <span>{t('navis_assistant', 'NAVIS Assistant')}</span>
          </span>
          <p className="text-h3 leading-7 font-semibold text-heading">
            {renderWithSrOnly(tDynamic(currentQuestion), currentQuestion)}
          </p>
      ...
      {thinking && (
        <div className="border border-hair bg-raised rounded-lg px-5 py-4 flex items-center gap-3">
          ...
          <span className="text-lead text-muted">{t('assistant_thinking', 'NAVIS Assistant is thinking…')}</span>
        </div>
      )}
        ...
        <div className="border border-hair bg-raised rounded-lg px-5 py-5 flex flex-col gap-4">
          <span className="flex items-center gap-2">
            ...
            <span className="text-label font-semibold uppercase tracking-[0.05em] text-accent">
              {renderWithSrOnly(t('ready_to_draft', 'READY TO DRAFT'), 'READY TO DRAFT')}
            </span>
          </span>
          <p className="text-lead leading-6 text-fg">
            {renderWithSrOnly(t('have_enough', 'I have enough to prepare the update.'), 'I have enough to prepare the update.')}
          </p>
          <Button variant="primary" block onClick={onReview}>
            {renderWithSrOnly(t('review_structured_update', 'Review Structured Update'), 'Review Structured Update')}
            <ArrowRight size={18} />
          </Button>
        </div>
      )}
      ...
      {stage === 'card' && turn && (
        <>
          <div className="flex flex-col gap-2">
            ...
            <p className="text-lead leading-6 text-fg">
              {t('extracted_data_msg', "Got it. I've extracted the structured data from your update. Please review it below.")}
            </p>
          </div>
          <StructuredCard
            slots={turn.slots}
        ...
        <div className="flex flex-col gap-2">
          ...
          {!fallback && (
            <Button variant="secondary" block onClick={onStartVoice}>
              <Mic size={18} />
              {renderWithSrOnly(t('answer_by_voice', 'Answer by voice'), 'Answer by voice')}
            </Button>
          )}
        </div>
      )}
