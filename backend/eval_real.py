...
and how its labels were produced.
    ...
    python backend/eval_real.py                 full report
    python backend/eval_real.py --wsdot-sheet   print the 13-pair verification sheet

Run `python backend/eval.py --schedule dataset/baseline_schedule_v2.json
...
def wsdot_linking_sheet(print_only=False):
    ...
    if not print_only:
        ...
        print()
        print("  Run `python backend/eval_real.py --wsdot-sheet` for the verification sheet.")
...
def constructcie():
    ...
    print()
    print("  What exists is `_compute_delay_reasons` (backend/server/main.py:2645), a")
...
#: `backend/server/delay_taxonomy.py :: DELAY_KEYWORDS`. It is a substring list, not a
