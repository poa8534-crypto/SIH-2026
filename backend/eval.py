...
Usage:
  python backend/eval.py                # calibrate on the full dataset, evaluate
  python backend/eval.py --cv           # 5-fold CV calibration for held-out metrics
"""
...
# as `decision.candidates[:3]` (backend/server/main.py:443, :1052, :3188), and the
...
# Since `Decision.top1` IS `candidates[0]` (backend/matching/models.py:96), the suggested
...
def print_recall_at_k(rows: list[dict], label: str = ""):
    ...
    print(f"  stored as candidates[:3] (backend/server/main.py:443) and the suggested id")
...
def main():
    ...
    ap.add_argument("--calibrate", action="store_true",
                    help="TUNING MODE: grid-search thresholds on the dev split "
                         ...
                         "backend/matching/config.py :: SHIPPED_THRESHOLDS, not a "
                         "description of the build that ships.")
    args = ap.parse_args()

    print("Loading schedule + ground truth ...")
    # The SAME configuration the server builds. `backend/server/main.py` calls
    ...
    print(f"   python backend/eval.py --schedule {args.schedule} \\")
    print(f"                  --ground-truth {args.ground_truth}")
    print(" Thresholds come from backend/matching/config.py :: SHIPPED_THRESHOLDS,")
    print(" which backend/server/main.py imports as MATCHING_THRESHOLDS. Quoting a")
