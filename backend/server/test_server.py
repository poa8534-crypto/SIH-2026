...
class TestIngestEndpoint:
    ...
    def test_ingest_dpr_text_file(self):
        dpr_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "dpr_day_01.txt"
    ...
    def test_ingest_piping_xlsx(self):
        xlsx_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "piping_progress.xlsx"
    ...
    def test_ingest_civil_xlsx(self):
        xlsx_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "civil_progress.xlsx"
        with open(xlsx_path, "rb") as f:
            response = client.post(
                "/ingest",
    ...
    def test_ingest_creates_job_record(self):
        dpr_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "dpr_day_03.txt"
        with open(dpr_path, "rb") as f:
            response = client.post(
                "/ingest",
...
class TestJobsEndpoint:
    ...
    def test_get_job_returns_events(self):
        # First ingest
        dpr_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "dpr_day_05.txt"
        with open(dpr_path, "rb") as f:
            response = client.post(
                "/ingest",
    ...
    def test_get_job_has_provenance(self):
        dpr_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "dpr_day_02.txt"
        with open(dpr_path, "rb") as f:
            response = client.post(
                "/ingest",
...
class TestScheduleExport:
    ...
    def test_pmxml_content_is_valid_xml(self):
        ...
        export_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "uploads" / filename
...
class TestCrossDPRStatistics:
    ...
    def test_ground_truth_coverage(self):
        ...
        gt_path = Path(__file__).resolve().parent.parent.parent / "dataset" / "ground_truth.csv"
...
class TestExportDownload:
    ...
    def test_non_export_extension_is_refused(self):
        ...
        upload_dir = Path(__file__).resolve().parent.parent.parent / "dataset" / "uploads"
