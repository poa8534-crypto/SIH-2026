...
## Corpus Check
- 231 files · ~610,364 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3357 nodes · 5460 edges · 290 communities (236 shown, 39 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 260 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9344f5c6`
...
### Community 1 - "main.py"
Cohesion: 0.05
Nodes (78): DurationDistribution, get, ProductivityMetric, _compute_delay_reasons(), _compute_duration_distribution(), _compute_productivity(), _compute_suggested_duration(), _cross_file_conflict() (+70 more)
...
Cohesion: 0.05
Nodes (64): DeclarativeBase, main(), Reset the demo database to a known seeded state. Safe to run while the server…, main(), Build a working database from scratch. Loads the 120-activity baseline…, Activity, _add_missing_columns(), AliasLexicon (+56 more)
...
### Community 5 - "Session"
Cohesion: 0.05
Nodes (61): AuditRecord, EventIndex, post, ResolveResponse, LinkedEvent, _now(), datetime, An extracted progress event linked to a schedule activity. Created by the… (+53 more)

### Community 6 - "interpret"
Cohesion: 0.07
Nodes (29): Protocol, _attr(), interpret(), llm_enabled(), llm_timeout_seconds(), LLMSuggestion, Optional LLM interpretation for one conversational turn. The LLM is an…, Keep only values that survive the deterministic validators. (+21 more)

### Community 7 - "test_matching.py"
Cohesion: 0.08
Nodes (31): _date_proximity(), Feature scoring per (event, candidate) pair — the precision-oriented stage.…, Returns (score, line_locked). A full line+size+spec match sets line_locked=True…, _tag_overlap(), Hybrid candidate retrieval: exact tag + BM25 + dense embeddings, fused with…, ActivityRecord, date, Schedule index: loads the baseline schedule and precomputes every lookup… (+23 more)
...
Cohesion: 0.07
Nodes (24): Extractor, date, Discipline, ExtractedEvent, Extract progress events from a source file. Dispatches to the appropriate…, Extract from a text-based daily progress report., Extract the report date from DPR header lines., Parse DPR text into logical progress spans. Returns list of (text, line_number)… (+16 more)

### Community 10 - "get_dashboard_data"
Cohesion: 0.07
Nodes (14): BaseHTTPRequestHandler, DashboardHandler, find_icon_file(), get_dashboard_data(), dashboard.py - Local web dashboard served on localhost:8080., Locate the extension's icon.svg across both run contexts. - Bundled in the…, Tests for dashboard.py - API endpoint and data retrieval., Regression: turns with model='' (empty string) must group as 'unknown'.… (+6 more)

### Community 11 - "scan"
Cohesion: 0.08
Nodes (29): aggregate_sessions(), _backfill_topics(), _ensure_column(), extract_agent_dispatch(), _extract_title(), is_subagent_record(), _meta_get(), _meta_set() (+21 more)

### Community 12 - "parse_jsonl_file"
Cohesion: 0.11
Nodes (15): parse_jsonl_file(), Parse a JSONL file and return (session_metas, turns, agents, line_count).…, _make_ai_title_record(), _make_assistant_record(), Test deduplication of streaming events by message.id., Multiple records with same message.id should produce one turn., Records with different message.id are separate turns., Records without message.id are kept as-is (no dedup). (+7 more)

### Community 13 - "Schedule.tsx"
Cohesion: 0.07
Nodes (28): ConfidenceBadge(), ConfidenceBadgeProps, DisciplineTag(), DISCIPLINES, isDiscipline(), PageHeader, PageHeaderContext, usePageHeader() (+20 more)
...
Cohesion: 0.12
Nodes (29): decide_outcome(), _rationale(), The matching engine: retrieval → feature scoring → calibrated decision.…, Core decision rule over scored candidates. Returns (outcome,…, Resolve one ExtractedEvent against the schedule., blend_with_line_lock(), compute_features(), final_score() (+21 more)

### Community 16 - "extractor.py"
Cohesion: 0.11
Nodes (31): Main extraction orchestrator. Ties together: 1. Deterministic pre-pass (regex)…, LLM backend interface for structured event extraction. Two backends behind one…, DateBasis, Discipline, EventStatus, ExtractionMethod, Enum, str (+23 more)

### Community 17 - "init_db"
Cohesion: 0.09
Nodes (14): init_db(), insert_turns(), Insert or update agent dispatch metadata. Last write wins per agent_id., upsert_agents(), upsert_sessions(), Tests for the CLI subagent summary lines in `today` and `stats`., TestCliSubagentLines, _turn() (+6 more)
...
### Community 21 - "_make_user_record"
Cohesion: 0.09
Nodes (15): _make_user_record(), Integration test: dedup across scan cycles., 3 streaming events for 2 messages should produce 2 turns., Re-scanning a file shouldn't create duplicate turns for same message_id., Test that updating a file only processes new lines (no double reads)., Growing a file must add only new turns, not re-insert old ones., Session totals should reflect all turns, not double-count., Last timestamp should advance after file grows. (+7 more)

### Community 22 - "types.ts"
Cohesion: 0.10
Nodes (27): ApiError, fetchWithHandler(), getBaseUrl(), FieldReports(), Filter, FILTERS, STATUS_TONE, when() (+19 more)

### Community 23 - "eval.py"
Cohesion: 0.16
Nodes (29): calibrate(), _dated(), _decision_for(), evaluate(), load_ground_truth(), main(), _parse_date(), pct() (+21 more)

### Community 24 - "_extract_one"
Cohesion: 0.10
Nodes (26): _extract_one(), parametrize, Belt and braces for providers with no grammar constraint., Tags feed the matcher's near-decisive tag_overlap feature. Description words…, The model returned ['steel erection', 'pipe rack'] and missed TK-1., The observed case: "All 12 pockets grouted" yields quantity 12 with no uom.…, Fix 2 must not weaken the forecast guard: it runs first and wins., Replays a fixed LLM output, standing in for a misbehaving model. (+18 more)

### Community 25 - "ScheduleIndex"
Cohesion: 0.08
Nodes (10): _predecessor_plausibility(), date, 1.0 — no predecessors, or all finished (with grace) by report date 0.7 —…, Path, Resolve a (possibly shortened) activity id to a schedule id. Handles ground-…, In-memory index over ~120 L5/L6 activities., ScheduleIndex, TestFeatures (+2 more)
...
Cohesion: 0.12
Nodes (14): DISCIPLINE_COLOR, DisciplineTagProps, DISCIPLINE_SHORT, clock(), FeedRow, FIELD_LABEL, PanelError(), position() (+6 more)

### Community 37 - "ExtractedEvent"
Cohesion: 0.18
Nodes (15): ExtractedEvent, Provenance, BaseModel, Where this event came from. Immutable once created., A single progress event extracted from a field report or spreadsheet. This is…, Tests for Pydantic schema validation., Ensure the schema rejects unknown fields (extra='forbid')., TestModels (+7 more)

### Community 38 - "HybridRetriever"
Cohesion: 0.14
Nodes (9): Path, _hashed_embeddings(), HybridRetriever, MiniLMEmbedder, ndarray, Exact/near-exact tag match, ranked by match quality: full (line+size+spec) >…, Returns (ordered candidate indices, per-candidate retrieval info)., sentence-transformers all-MiniLM-L6-v2, offline-first. (+1 more)
...
### Community 48 - "test_llm_guards.py"
Cohesion: 0.16
Nodes (16): _env(), make_backend_from_env(), NullBackend, Read config with precedence: real environment, then .env, then default. The…, Build the extraction backend named by EXTRACTION_PROVIDER. Defaults to rules-…, No-op backend that returns empty LLM output. Useful for testing prepass-only., Guards protecting the pipeline from LLM extraction errors. These cover three…, The guard must not suppress genuine actuals. (+8 more)
...
### Community 53 - "_build_event"
Cohesion: 0.13
Nodes (11): _build_event(), ExtractedEvent, Turn a labelled mention into an ExtractedEvent via the shared prepass., _basis_at(), DateBasis, Decide whether this line asserts a start date, a finish date, both, or neither,…, Basis of the i-th date in a prepass hint dict, defaulting to EXPLICIT for hand-…, Every date has to know how it was obtained. A date the span carried is an… (+3 more)
...
Cohesion: 0.16
Nodes (7): Roll one event onto one node and return the RollupResult., _rollup_one(), MatchingEngine, Cosine similarity of the dense channel for this candidate (None if the…, fixture, A node with no planned quantity AND no unit of measure is a milestone: a…, TestMilestoneNode
...
### Community 57 - "get_db"
Cohesion: 0.26
Nodes (6): get_db(), _make_custom_title_record(), Existing DBs without message_id column should be upgraded., One-time backfill of topics for DBs that predate topic support (#147)., Simulate a not-yet-backfilled DB: clear the captured topic and the one-time…, TestTopicBackfill
...
### Community 65 - "read_text"
Cohesion: 0.19
Nodes (11): Path, Reading source documents without losing characters. The supplied DPRs are…, Read a text document, preserving every character it actually contains., Read a JSON document. Same rules, named separately for intent., read_json_text(), read_text(), db_session(), Shared fixtures. `server/test_server.py` builds its own database with an… (+3 more)

### Community 66 - "errorDetail"
Cohesion: 0.21
Nodes (12): NeedsYourResponse(), RecentUpdates(), STATUS_ICON, STATUS_SHORT, when(), PLANNER, errorDetail(), Card() (+4 more)
...
### Community 91 - "LLMBackend"
Cohesion: 0.20
Nodes (6): ABC, Load baseline schedule and build compact context string., LLMBackend, Check if this backend is reachable., Abstract LLM backend for structured event extraction., Extract structured events from text spans. Args: text_spans: Free-text lines to…

### Community 92 - "project_name_from_cwd"
Cohesion: 0.33
Nodes (3): project_name_from_cwd(), Derive a friendly project name from cwd path., TestProjectNameFromCwd

### Community 93 - "OpenAICompatibleBackend"
Cohesion: 0.27
...
### Community 94 - "extract_quantities"
Cohesion: 0.27
Nodes (6): extract_quantities(), _normalize_uom(), Extract all (quantity, uom) pairs from free text., Normalize unit-of-measurement strings., Tests for quantity + UOM patterns., TestQuantityExtraction
...
### Community 96 - "api"
Cohesion: 0.29
Nodes (8): FieldNav(), TABS, api, CLARIFICATION, enterConversation(), reachReady(), REPORT, wrap()
...
Cohesion: 0.33
Nodes (6): Current Modification Area, Current path, Downstream, Known follow-ups, Upstream, Verification performed
...
## Knowledge Gaps
- **1144 isolated node(s):** `name`, `version`, `license`, `private`, `type` (+1139 more)
...
- **Why does `Discipline` connect `extractor.py` to `main.py`, `Activity`, `ExtractedEvent`, `test_matching.py`, `Extractor`, `infer_discipline`, `test_llm_guards.py`, `LLMEventOutput`, `RollupAccumulator`, `_extract_one`, `SpreadsheetParser`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
- **Are the 26 inferred relationships involving `Activity` (e.g. with `main()` and `_activity_model()`) actually correct?**
  _`Activity` has 26 INFERRED edges - model-reasoned connections that need verification._
