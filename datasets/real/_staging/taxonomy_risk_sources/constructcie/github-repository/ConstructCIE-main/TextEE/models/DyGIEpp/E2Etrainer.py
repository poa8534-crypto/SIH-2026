...
from utils import print_dev_scores
import ipdb
...
class DyGIEppE2ETrainer(BasicTrainer):
    ...
    def load_tokenizer_(self, checkpoint=None):
        if checkpoint:
            ...
            state = torch.load(os.path.join(checkpoint, "best_model.tokenizer"), weights_only=False)
    ...
    def load_model_(self, checkpoint=None):
        ...
        if checkpoint:
            ...
            state = torch.load(os.path.join(checkpoint, "best_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        _SCORE_KEY_MAP = {
            'event_role': 'role', 'event_trigger': 'trigger',
            'trigger': 'trigger', 'role': 'role',
            'entity': 'entity', 'relation': 'relation',
        }
        target_task = self.config.target_task
        score_key = _SCORE_KEY_MAP.get(target_task, target_task)
        best_scores = {score_key: {"f": 0.0}}
        best_epoch = -1
        ...
        for epoch in range(self.config.max_epoch):
            ...
            print_dev_scores(dev_scores)

            if dev_scores[score_key]['f'] >= best_scores[score_key]['f']:
                best_scores = {score_key: dev_scores[score_key]}
