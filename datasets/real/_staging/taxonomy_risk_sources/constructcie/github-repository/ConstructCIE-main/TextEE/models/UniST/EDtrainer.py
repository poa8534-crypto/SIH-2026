...
class UniSTEDTrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            span_state = torch.load(os.path.join(checkpoint, "best_span_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        span_state = torch.load(os.path.join(self.config.output_dir, "best_span_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
        self.span_model.load_state_dict(span_state['span_model'])
