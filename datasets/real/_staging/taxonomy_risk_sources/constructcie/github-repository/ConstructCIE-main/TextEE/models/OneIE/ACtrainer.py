...
from .E2Etrainer import OneIEE2ETrainer
...
class OneIEACTrainer(E2EACMixin, OneIEE2ETrainer):
    """OneIE trainer for the AC (Accident Classification) task."""
        ...
        param_groups = [
            ...
            },
            {
                'params': [p for n, p in self.model.named_parameters()
                           if not n.startswith('bert') and 'crf' not in n and 'global_feature' not in n],
                'lr': self.config.learning_rate, 'weight_decay': self.config.weight_decay
            },
            {
                'params': [p for n, p in self.model.named_parameters()
                           if not n.startswith('bert') and ('crf' in n or 'global_feature' in n)],
    ...
    def _to_ie_format(self, pred):
        """Convert OneIE event-tuple output to standard IE dict format."""
        entity_mentions = []
        event_mentions  = []
        seen_spans      = set()
    ...
    def predict(self, data, **kwargs):
        """E2E prediction returning standard IE format."""
