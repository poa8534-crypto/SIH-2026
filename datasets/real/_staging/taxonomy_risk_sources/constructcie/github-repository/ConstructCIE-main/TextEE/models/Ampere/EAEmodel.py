...
class Degree(nn.Module):
    ...
    def process_data(self, batch):
        ...
        if os.path.basename(str(self.config.pretrained_model_name)).startswith('bart'):
    ...
    def generate(self, input_ids, attention_mask, num_beams=4, max_length=50, **kwargs):
        ...
        for bid in range(len(input_ids)):
            # if os.path.basename(str(self.config.pretrained_model_name)).startswith('google/t5') or os.path.basename(str(self.config.pretrained_model_name)).startswith('t5'):
...
class DegreeCopyReg(Degree):
    def __init__(self, config, tokenizer):
        ...
        logger.info(f'Using model {self.__class__.__name__}')
        logger.info(f'Loading pre-trained model {config.pretrained_model_name}')
        if os.path.basename(str(config.pretrained_model_name)).startswith('bart-'):
            self.model_config =  AutoConfig.from_pretrained(config.pretrained_model_name, cache_dir=config.cache_dir)
...
class AMRPrefixGenBase(nn.Module):
    ...
    def load_model(self, load_path):
        ...
        self.wte.load_state_dict(torch.load(os.path.join(load_path, "wte.mdl"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False))
        self.AMR_model.load_state_dict(torch.load(os.path.join(load_path, "amrmodel.mdl"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False))
        if self.model_config.use_encoder_prefix:
