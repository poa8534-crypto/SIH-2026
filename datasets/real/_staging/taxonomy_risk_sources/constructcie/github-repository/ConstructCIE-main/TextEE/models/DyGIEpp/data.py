        ...
        for inst in self.raw_data:
            ...
            for wid in encoding.word_ids():
                if wid != prev_wid:
                    ...
                    token_lens[-1] += 1
            if not token_lens or 0 in token_lens:
                raise ValueError
            inst['pieces'] = pieces
            ...
            if self.max_length != -1 and len(pieces) > self.max_length - 2:
                ...
                ent_end_map = {e['id']: e['end'] for e in inst['entity_mentions']}
                inst['entity_mentions'] = [e for e in inst['entity_mentions'] if e['end'] <= max_token_include]
                inst['event_mentions'] = [
                    {**e, 'arguments': [a for a in e['arguments']
                                        if ent_end_map.get(a['entity_id'], 0) <= max_token_include]}
                    for e in inst['event_mentions']
                    if e['trigger']['end'] <= max_token_include
                ]
                overlength_num += 1
                logger.info('Truncated overlength instance: doc_id={} wnd_id={} (pieces={})'.format(
                    inst.get('doc_id', '?'), inst.get('wnd_id', '?'), len(pieces)))
            self.data.append(inst)

        if overlength_num:
            logger.info('Truncated {} overlength instances'.format(overlength_num))
