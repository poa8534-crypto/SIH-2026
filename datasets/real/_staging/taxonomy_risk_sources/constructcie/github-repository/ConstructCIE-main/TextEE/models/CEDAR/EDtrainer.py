...
class CEDAREDTrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            ti_state = torch.load(os.path.join(checkpoint, "best_ti_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
            etr_state = torch.load(os.path.join(checkpoint, "best_etr_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
            etc_state = torch.load(os.path.join(checkpoint, "best_etc_model.state"), map_location=f'cuda:{self.config.gpu_device}', weights_only=False)
